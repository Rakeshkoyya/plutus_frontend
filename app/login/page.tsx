    ...
    <div className="relative flex min-h-screen items-center justify-center bg-background p-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="mb-8 text-center">
          <Link href="/" className="mx-auto mb-4 flex h-14 w-14 items-center justify-center">
            <BrandMark className="h-14 w-14 rounded-2xl shadow-pop" iconClassName="h-7 w-7" />
          </Link>
          <h1 className="text-2xl font-bold text-foreground">Plutus</h1>
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-card">
          ...
          {config?.google_enabled && (
            <>
              ...
              <a
                href={`${api.apiUrl}/api/auth/google/login`}
                className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-border bg-card text-sm font-medium text-foreground hover:bg-muted"
              >
          ...
          {!config?.google_enabled && (
            <p className="mt-5 rounded-lg bg-muted px-3 py-2 text-center text-xs text-muted-foreground">
              Google sign-in appears here once <code>GOOGLE_CLIENT_ID</code> /{" "}
              <code>SECRET</code> are set in the backend env.
            </p>
          )}
        </div>

        <p className="mt-6 text-center text-xs text-muted-foreground">
          Seeded admin &middot; <span className="font-medium text-foreground">admin / admin123</span>
        </p>
      </div>
    </div>
  );
